...
class Solargraph(LanguageServer):
    ...
    def setup_runtime_dependencies(self, logger: MultilspyLogger, config: MultilspyConfig, repository_root_path: str) -> str:
        ...
        try:
            result = subprocess.run(["gem", "list", "^solargraph$", "-i"], check=False, shell=True, capture_output=True, text=True, cwd=repository_root_path)
            if result.stdout.strip() == "false":
                logger.log("Installing Solargraph...", logging.INFO)
                subprocess.run(dependency["installCommand"].split(), check=True, shell=True, capture_output=True, cwd=repository_root_path)
        except subprocess.CalledProcessError as e:
            raise RuntimeError(f"Failed to check or install Solargraph. {e.stderr}")

        # Get the solargraph executable path
        try:
            result = subprocess.run([which_cmd, "solargraph"], check=True, shell=True, capture_output=True, text=True, cwd=repository_root_path)
            executeable_path = result.stdout.strip()
            executeable_path = "D:\\Env\\Ruby33-x64\\bin\\solargraph.bat"
